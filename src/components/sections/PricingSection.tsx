              ...
              <div className={`p-10 md:p-12 rounded-[40px] border transition-all duration-500 bg-bg-card flex flex-col h-full hover:shadow-glow ${plan.featured ? 'border-brand-main border-[3px] md:scale-105 z-10' : 'border-border-subtle hover:border-brand-main'}`}>
                <div className="mb-12">
                   ...
                   <div className="flex items-baseline gap-1">
                      {plan.price !== 'Custom' && <span className="text-[20px] md:text-[24px] font-bold text-text-heading">$</span>}
                      <span className="text-[42px] sm:text-[50px] md:text-[65px] font-bold text-text-heading tracking-tighter">{plan.price}</span>
                      {plan.price !== 'Custom' && <span className="text-[16px] md:text-[18px] font-medium text-text-muted ml-2">/ project</span>}
                   </div>
                </div>

                <p className="text-[17px] md:text-[18px] text-text-body mb-12 leading-relaxed">
                   {plan.description}
                </p>
                ...
                <div className="space-y-6 mb-16 flex-grow">
                   {plan.features.map((feature, j) => (
                     <div key={j} className="flex items-center gap-4">
                        ...
                        <span className="text-[15px] md:text-[16px] font-medium text-text-body">{feature}</span>
                     </div>
                   ))}
                </div>
